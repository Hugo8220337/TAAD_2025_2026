...
def main(ndays, all_flag, outdir):
    global OUTPUT_DIR
    if outdir:
        OUTPUT_DIR = outdir
    # garantir que o diretório existe desde já
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    if all_flag:
        print("Executar para todos os dias — saída em: " + OUTPUT_DIR)
        retrieve_all()
    else:
        print(f"Executar para {ndays} dias — saída em: " + OUTPUT_DIR)
...
if __name__ == "__main__":
    ...
    p.add_argument("--outdir", type=str, help="diretório para gravar ficheiros CSV (default icnf_data)")
    args = p.parse_args()
