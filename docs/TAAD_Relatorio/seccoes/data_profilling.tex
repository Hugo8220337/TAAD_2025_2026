...
\subsection{Unicidade dos dados}
Algo bastante importante de averiguar no dataset, é se existem registos duplicados, visto que isso poluiria o data warehouse, podendo atrapalhar a análise dos dados, ou, mais importante, se existe algum id repetido.

Após analisar todos os registos, foi possível constar que para além de não existirem ids repetidos, também não existem registos inteiramente iguais entre si, confirmando assim a unicidade do dataset. Isto indica também que a origem dos dados (ICNF) mantém um controlo adequado sobre as entradas dos registos, eliminando assim a necessidade de operações de deduplicaçao dos dados no processo de integração, e reforça a fiabilidade de análises posteriores, dado que não vão existir duplicações que poderão distorcer as estatísticas.
